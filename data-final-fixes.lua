...
DCM.ITEM_TIER = {
    [4] = {
        "wood", "iron-ore", "copper-ore", "stone", "coal", "iron-plate",
        "copper-plate", "steel-plate", "stone-brick"
    },
    [5] = {
        "copper-cable", "iron-gear-wheel", "iron-stick", "sulfur",
        "plastic-bar", "solid-fuel", "electronic-circuit", "advanced-circuit"
    }
}
DCM.TIERS = 5
DCM.TIER_COLOURS = {
...
for tier = 1, DCM.TIERS do
    DCM.create_machine_entity(tier, nil, nil, nil, nil, nil,
                              (tier < DCM.TIERS) and
                                  (DCM.MACHINE_PREFIX .. (tier + 1)) or nil, nil)
end

for tier = 4, DCM.TIERS do DCM.create_machine_item(tier) end

DCM.create_machine_recipe(4, {
    {"deadlock-crating-machine-3", 1}, {DCM.LOADER_FOUR, 4}, {"steel-plate", 10}
})

DCM.create_machine_recipe(5, {
    {"deadlock-crating-machine-4", 1}, {DCM.LOADER_FIVE, 4}, {"steel-plate", 10}
})

data.raw.recipe[DCM.MACHINE_PREFIX .. "1"].ingredients =
    {{DCM.LOADER_ONE, 8}, {"steel-plate", 10}}

data.raw.recipe[DCM.MACHINE_PREFIX .. "2"].ingredients =
    {{DCM.MACHINE_PREFIX .. "1", 1}, {DCM.LOADER_TWO, 4}, {"steel-plate", 10}}

data.raw.recipe[DCM.MACHINE_PREFIX .. "3"].ingredients =
    {{DCM.MACHINE_PREFIX .. "2", 1}, {DCM.LOADER_THREE, 4}, {"steel-plate", 10}}

function getCraftingSpeed(level)
    return
        level * settings.startup["bobmods-logistics-beltspeedperlevel"].value /
            15
end

if settings.startup["bobmods-logistics-beltoverhaulspeed"] and
    settings.startup["bobmods-logistics-beltoverhaulspeed"].value then
    -- yellow
    if data.raw["assembling-machine"][DCM.MACHINE_PREFIX .. "1"] then
        data.raw["assembling-machine"][DCM.MACHINE_PREFIX .. "1"].crafting_speed =
            getCraftingSpeed(2)
    end
    -- red
    if data.raw["assembling-machine"][DCM.MACHINE_PREFIX .. "2"] then
        data.raw["assembling-machine"][DCM.MACHINE_PREFIX .. "2"].crafting_speed =
            getCraftingSpeed(3)
    end
    -- blue
    if data.raw["assembling-machine"][DCM.MACHINE_PREFIX .. "3"] then
        data.raw["assembling-machine"][DCM.MACHINE_PREFIX .. "3"].crafting_speed =
            getCraftingSpeed(4)
    end
    -- purple
    if data.raw["assembling-machine"][DCM.MACHINE_PREFIX .. "4"] then
        data.raw["assembling-machine"][DCM.MACHINE_PREFIX .. "4"].crafting_speed =
            getCraftingSpeed(5)
    end
    -- green
    if data.raw["assembling-machine"][DCM.MACHINE_PREFIX .. "5"] then
        data.raw["assembling-machine"][DCM.MACHINE_PREFIX .. "5"].crafting_speed =
            getCraftingSpeed(6)
    end
end
...
for tier = 1, DCM.TIERS do
    if data.raw["assembling-machine"][DCM.MACHINE_PREFIX .. tier] then
        data.raw["assembling-machine"][DCM.MACHINE_PREFIX .. tier]
        .fast_replaceable_group = "crating-machine"
        if (tier < 5) then
            data.raw["assembling-machine"][DCM.MACHINE_PREFIX .. tier]
            .next_upgrade = DCM.MACHINE_PREFIX .. tier + 1
        else
            data.raw["assembling-machine"][DCM.MACHINE_PREFIX .. tier]
            .next_upgrade = ""
        end
